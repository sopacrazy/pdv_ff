    ...
    <AppShell rotaAtiva="/admin">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        <button
          ...
          className="bg-white p-8 rounded-2xl shadow-sm flex flex-col items-center justify-center gap-4 text-slate-500 h-56 border border-slate-200/80 hover:border-slate-300 transition-colors group"
        >
          <Settings size={48} className="text-slate-300 group-hover:text-slate-400 transition-colors" />
          <span className="text-xl font-bold text-slate-700">Configuração</span>
          <span className="text-slate-400 text-sm">Loja e caixa</span>
        </button>
      </div>
    </AppShell>
  );
