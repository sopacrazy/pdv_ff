...
function criarDb() {
  ...
  db.exec(`
    CREATE TABLE vendas (
      id TEXT PRIMARY KEY, numero_cupom TEXT, loja TEXT, caixa TEXT, operador TEXT,
    ...
    CREATE TABLE usuarios (id TEXT PRIMARY KEY, nome TEXT, protheus_vend_codigo TEXT, protheus_vend_nome TEXT);
  `);
  return db;
}
...
function inserirVenda(db, id, overrides = {}) {
  db.prepare(`
    INSERT INTO vendas (id, numero_cupom, loja, caixa, operador, cliente_nome, cliente_cpf, subtotal, desconto, total, forma_pagamento, criado_em, data_local)
    VALUES (@id, '000001', '01', '001', 'Operador Teste', 'Cliente', '', 33000, 0, 33000, '033', @criado_em, '2026-09-23')
  ...
  // Um único operador reaproveitado entre vendas — cada uma cadastrando de novo duplicaria o nome
  // e quebraria a busca por "operador único" em enviarVendaAoProtheus.
  db.prepare(`INSERT OR IGNORE INTO usuarios (id, nome, protheus_vend_codigo, protheus_vend_nome) VALUES ('usuario-teste', 'Operador Teste', '000090', 'Vendedor Teste')`)
    .run();
}

// Mocka as duas consultas feitas por prepararVenda4Sales (cliente + tabela de preços) e o POST final.
function mockFetchSucesso(vendaId) {
  return async (url) => {
    ...
    if (href === URL_TESTE_4SALES) {
      return new Response(JSON.stringify({ idWeb: vendaId, company: '14', branch: '01', status: 'EFE', ticket: 'TESTE01' }), { status: 200 });
    }
...
test('processarFilaProtheus envia as pendentes e para no primeiro sinal de "sem internet"', async () => {
  await comCredenciais(async () => {
    ...
    globalThis.fetch = async (url, opts) => {
      chamada += 1;
      if (chamada <= 2) return fetchSucesso(url, opts); // as 2 consultas de v1 (cliente + preços)
      if (chamada === 3) return fetchSucesso(url, opts); // POST de v1
